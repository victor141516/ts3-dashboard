<script lang="ts">
  ...
  $: isServerEmpty = Object.values(serverSummary ?? {}).reduce((acc, cur) => acc + cur.length, 0) === 0
  ;(async () => {
    while (true) {
...
<main>
  <button class="absolute top-0 left-0 mt-8 ml-8 text-6xl" on:click={() => (isSoundEnabled = !isSoundEnabled)}>
    {isSoundEnabled ? '🔊' : '🔇'}
  </button>
  <!-- <Tumbleweed /> -->
  <div class="h-screen w-screen flex flex-col items-center justify-center pb-[20vh]">
    <div class="flex flex-col items-center">
      ...
      <div class="border rounded-full relative">
        <button on:click={() => (viewerName = '')} class="absolute top-[-3px] right-[-3px]">
          <img src={clear} alt="clear input" class="scale-90" />
        </button>
        <input type="text" name="viewer-name" class="px-2 rounded-full outline-none" bind:value={viewerName} />
      </div>
    </div>
        ...
        {:else}
          ...
          {#each Object.entries(serverSummary) as [channel, users]}
            {#if users.length > 0 && channel !== VIEWERS_CHANNEL_NAME}
              <li class="mt-3">
                <span class="font-bold"><button on:click={() => composeMessage(channel)}>💬</button> {channel}</span>
                <ul>
                  {#each users as user}
                    <li>🔵 {user}</li>
                  {/each}
